window.HemoAuth = (() => {
  ...
  function demoProfile() {
    return (
      JSON.parse(localStorage.getItem("hemo_demo_profile") || "null") || null
    );
  }

  function makeDemoProfile(email = "demo@hemoatlas.local") {
    const role = email.includes("curator")
      ? "curator"
  ...
  async function login(email, password) {
    if (isDemoMode()) {
      ...
      currentProfile = profile;
      HemoApp.toast(
        "Login demo realizado. Configure o Supabase para autenticação real.",
      );
      setTimeout(() => (location.href = "perfil.html"), 300);
      return;
    }

    const { error } = await HemoSupabase.client.auth.signInWithPassword({
      email,
      password,
    });
...
async function signup(input) {
  ...
  async function refreshAuthLabel(el) {
    ...
    el.textContent = currentUser
      ? currentProfile?.full_name || currentUser.email || "Perfil"
      : "Entrar";
